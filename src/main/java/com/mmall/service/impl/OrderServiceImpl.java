...
public class OrderServiceImpl implements IOrderService {
    ...
    public ServerResponse pay(Integer userId, Long orderNo, String path) {
        ...
        String subject = new StringBuilder().append("lzxmmall扫码支付,订单号:").append(outTradeNo).toString();
        ...
        for (OrderItem orderItem : orderItemList) {
            GoodsDetail goodsDetail = GoodsDetail.newInstance(orderItem.getProductId().toString(),
                    orderItem.getProductName(), BigDecimalUtil.mul(orderItem.getCurrentUnitPrice().
                            doubleValue(), new Double(100)).longValue(), orderItem.getQuantity());
    ...
    public ServerResponse aliCallback(Map<String, String> params) {
        ...
        if (order.getStatus() >= Const.OrderStatusEnum.PAID.getCode()) {
            //返回成功，这样controller层进行判断才能返回success给支付宝，从而避免了下一次调用
